...
class ExportarExcel implements FromCollection, WithHeadings, WithCustomStartCell, WithTitle, WithStyles
{
    ...
    public function reporteCumpleanos()
    {
        ...
        $data = Membrecia::select(
            ...
            ->havingRaw("EXTRACT(doy FROM fecha_nacimiento)>=EXTRACT(doy FROM to_date('" . Carbon::parse($this->fechaInicial)->format('Ymd') . "','YYYYMMDD')) and EXTRACT(doy FROM fecha_nacimiento)<=EXTRACT(doy FROM to_date('" . Carbon::parse($this->fechaFinal)->format('Ymd') . "','YYYYMMDD'))")
            ->orderBy('diaanonacimiento', 'asc')
            ->groupBy('id')
            ->get();
        return $data;
